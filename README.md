...
Диаризация разделяет аудио на части по спикерам и таймкодам и показывает, кто из участников говорил в каждом фрагменте. На выходе получается разметка разговора, которую удобно использовать для протоколов и аналитики коммуникаций.
...
Определяет эмоциональную окраску речи в аудио (и умеет анализировать текст): возвращает отсортированный список классов с вероятностями, где первый элемент — самая вероятная тональность. Классы: `Very Negative`, `Negative`, `Neutral`, `Positive`, `Very Positive`
